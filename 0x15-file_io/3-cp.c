...
int main(int ac, char **av)
{
	...
	if (ac != 3)
		printf(STDERR_FILENO, USAGE), exit(97);
	from_fd = open(av[1], O_RDONLY);
	if (from_fd == -1)
		printf(STDERR_FILENO, ERR_NOREAD, av[1]), exit(98);
	to_fd = open(av[2], O_WRONLY | O_CREAT | O_TRUNC, PERMISSIONS);
	if (to_fd == -1)
		printf(STDERR_FILENO, ERR_NOWRITE, av[2]), exit(99);
	...
	to_fd = close(to_fd);
	if (from_fd)
		printf(STDERR_FILENO, ERR_NOCLOSE, from_fd), exit(100);
	if (to_fd)
		printf(STDERR_FILENO, ERR_NOCLOSE, from_fd), exit(100);
